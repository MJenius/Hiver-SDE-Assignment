...
This report documents the final evaluation of an autonomous customer support agent for `@AppleSupport`. Moving beyond exploratory prototypes and ungrounded claims, Phase 3 establishes an empirically hardened benchmark governed by five strict principles:
...
| | Macro-F1 | **0.8668** | [0.8228, 0.9047] | 0.0215 | Class-balanced diagnostic macro average |
...
| | Autonomous Coverage | **43.50%** | [37.00%, 50.50%] | 0.0350 | Stratified test benchmark ($N=87 / 200$) |
| | Natural Stream Coverage | **27.93%** | [25.70%, 30.20%] | 0.0115 | Unconstrained validation traffic ($N=1,500$) |
| | False Auto-Handle Rate (FAHR) | **6.90%** | [2.30%, 12.64%] | 0.0273 | Adjudicated risk handoffs missed ($N=6 / 87$) |
| **Multi-Stage Retrieval** | Intent-Match Top-5 MRR | **0.1000** | [0.0600, 0.1400] | 0.0204 | Target intent category match (not resolution precedent) |
| **Claim Grounding** | Claim Support Rate | **100.00%** | [100.00%, 100.00%] | 0.0000 | Evaluated among auto-handled replies with claims ($N=87$) |
...
### 2.2 Autonomous Coverage (43.50% vs. 27.93%)
...
* **Dataset Composition**: 43.5% is measured on a class-stratified 200-case benchmark. In unconstrained natural inbound traffic, coverage drops to **27.93%** due to ambient greetings and rants.
